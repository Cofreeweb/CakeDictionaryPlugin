 ...
 */
class ExtractNodesShell extends AppShell {
 ...
 */
	...
	{
    foreach( $this->_translations ['LC_MESSAGES'] as $domain => $nodes)
    {
      foreach( $nodes as $node => $info)
      {
        $this->Dictionary->add( $node, $domain, $info ['references'], $info ['msgid_plural']);
      }
    }
	}
